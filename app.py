...
#API key connection
def Api_connect():
    Api_Id="AIzaSyCRlE3-Ly-iitIQrSWM_M3zF-E6BNR9IXQ"
...
#MongoDB Connection
client = pymongo.MongoClient("mongodb+srv://sabarishraja:youtube@youtube.u3ogmrx.mongodb.net/?retryWrites=true&w=majority")
...
class HomePage:
    def show(self):
        ...
        if st.button(":white[welcome]"):
            st.balloons()
            st.write({"Name": "Sabarish"}, 
                     {"Batch No": "D104"}, 
                     {"Project": "Youtube Data Harvesting"})

# ... (previous code)

class ProjectPage:
    def show(self):
        st.title("Welcome to :red[Project Section]🎗️")
...
class AccountPage:
    def show(self):
        st.title("Welcome to :violet[Profile]😎")
...
class MultiPageApp:
    ...
    def run(self):
        st.title("Project 1")
        options = st.sidebar.radio("Main Menu", ["Home", "Project", "Account"])
        # Display the selected page
        selected_page = self.pages.get(options, HomePage())
        selected_page.show()

if __name__ == "__main__":
    st.set_page_config(page_title="Streamlit MultiPage Example")
